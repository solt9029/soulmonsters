...
export type SelectAsHamontakiTargetActionPayload = {
  targetGameCard: GameCardModel;
};

export function handleSelectAsHamontakiTarget(
  userId: string,
  payload: SelectAsHamontakiTargetActionPayload,
  gameModel: GameModel,
): GameModel {
  ...
  if (resolvingGameChainLink.effect.type !== EffectType.HAMONTAKI_SPECIAL_SUMMON) {
    throw new Error('Resolving chain link is not HAMONTAKI_SPECIAL_SUMMON');
  }
