...
import { packBattlePositions } from './packBattlePositions';

describe('packBattlePositions', () => {
  it('should pack battle positions after removing a card', () => {
    ...
    const result = packBattlePositions(gameModel, 'user1', 1);
  ...
  it('should only affect cards belonging to specified user', () => {
    ...
    const result = packBattlePositions(gameModel, 'user1', 0);
  ...
  it('should only affect cards in BATTLE zone', () => {
    const gameModel = new GameModel({
      id: 1,
      gameCards: [
        new GameCardModel({
